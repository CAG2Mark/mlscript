...
class HandlerLowering(paths: HandlerPaths, opt: Opt[EffectHandlers])(using TL, Raise, Elaborator.State, Elaborator.Ctx, Config):
  ...
  private def translateBlock(nme: String, blk: Block, h: HandlerCtx, scopedVars: collection.Set[ScopedSymbol], extraRestoreVars: List[ValueSymbol]): Block =
    ...
    if oneState && !parts.containsError && !needsStackSafety then
      return b
    val vars = extraRestoreVars ::: (if debugEnabled then ctx.resumeInfo.currentLocals else computeRestoreList(parts))
  ...
  private def postTranslateTopLevelCtx(b: Block)(using HandlerCtx): Block =
    postTranslateIllegalEffectCtx(b, Call.raw(paths.topLevelEffectPath, (Value.Lit(Tree.BoolLit(debugEnabled)).asArg :: Nil) ne_:: Nil)(CallMetadata.defaultMlsFun), true, opt.flatMap(_.stackSafety.map(_.stackLimit)))
