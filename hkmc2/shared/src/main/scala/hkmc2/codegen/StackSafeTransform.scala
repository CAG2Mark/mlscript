...
class StackSafeTransform(depthLimit: Int)(using State):
  ...
  private val geqPath = State.builtinOpsMap(">=").asPath
  private val addPath = State.builtinOpsMap("+").asPath
  ...
  def transform(b: Block): Block = b match
    ...
    case Define(d: FunDefn, rest) => Define(rewriteFn(d), transform(rest))
  ...
  def rewriteFn(defn: FunDefn) =
    ...
    val newBlk = transform(defn.body)
    ...
    val blk = blockBuilder
      ...
      .ifthen(
        ...
        blockBuilder.assign( // tmp = perform(undefined)
          TempSymbol(None, "tmp"), 
          Call(Select(stackHandlerPath, Tree.Ident("perform"))(N), List(Value.Lit(Tree.UnitLit(true)).asArg))(true)).end())
      .rest(newBlk)

    FunDefn(defn.sym, defn.params, blk)
  ...
  def transformTopLevel(b: Block) =
    ...
    val handle = HandleBlock(
      ...
      List(Handler(
        BlockMemberSymbol("resume", Nil), resumeSym, Nil,
        blockBuilder
      ...
      blockBuilder
        ...
        .rest(transform(b)), // transform the rest of the body
