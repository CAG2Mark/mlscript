...
lambda1 = class lambda {
  ...
  static #sum0;
  static {
    let tmp, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, tmp8, tmp9, tmp10, tmp11, tmp12, tmp13, tmp14, tmp15, tmp16, tmp17, tmp18, tmp19, tmp20, tmp21, tmp22, tmp23, tmp24, tmp25, tmp26, tmp27, tmp28, tmp29, tmp30, tmp31, tmp32, tmp33, tmp34;
  ...
  static apply(t4, a3) {
    let param0, param1, param01, param11, x1, b2, e;
    if (t4 instanceof lambda.Thunk.class) {
      ...
      if (param0 instanceof lambda.Lam.class) {
        ...
        return lambda.myBind(lambda.#myGet, (orig) => {
          let tmp1, tmp2, tmp3;
          tmp1 = lambda.Thunk(a3, orig);
          tmp2 = lambda.traverseTerm(b2);
          tmp3 = lambda.pushVar(x1, tmp1, tmp2);
          return lambda.withEnv(e, tmp3)
        })
      } else {
        throw new globalThis.Error("match error");
      }
  ...
  static eval(ter) {
    let param0, i, param01, param1, param2, c, a4, b2, param02, param11, u, v2, param03, param12, x1, b3, param04, param13, t5, e, param05, param14, u1, v3, param06, x2, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6;
    if (ter instanceof lambda.Var.class) {
      param06 = ter.s;
      x2 = param06;
      return lambda.myBind(lambda.#myGet, (e1) => {
        let tmp7;
        tmp7 = lambda.lookupVar(x2);
        return lambda.myBind(tmp7, (t6) => {
      ...
      return lambda.myBind(tmp1, (u_) => {
        let tmp7;
        tmp7 = lambda.traverseCon(v3);
        return lambda.myBind(tmp7, (v_) => {
          let tmp8, tmp9;
          tmp8 = u_ + v_;
          tmp9 = lambda.Con(tmp8);
