...
picChar = function picChar(c) {
  let lscomp, scrut, scrut1, scrut2, scrut3, scrut4, scrut5, tmp493, tmp494, tmp495, tmp496, tmp497, tmp498, tmp499, tmp500, tmp501, tmp502, tmp503, tmp504, tmp505;
  lscomp = function lscomp(ls) {
    let param0, param1, h, t, first1, first0, c_, letter, scrut6, tmp506;
    if (ls instanceof NofibPrelude.Nil.class) {
      ...
      if (globalThis.Array.isArray(h) && h.length === 2) {
        ...
        if (scrut6 === true) {
          tmp506 = lscomp(t);
          return NofibPrelude.Cons(letter, tmp506)
        } else {
          return lscomp(t)
        }
      } else {
  ...
  if (scrut5 === true) {
    tmp493 = NofibPrelude.int_of_char(c);
    tmp494 = NofibPrelude.int_of_char("A");
    tmp495 = tmp493 - tmp494;
    return NofibPrelude.atIndex(tmp495, alphas)
  } else {
    scrut4 = isLower(c);
    if (scrut4 === true) {
      tmp496 = NofibPrelude.int_of_char(c);
      tmp497 = NofibPrelude.int_of_char("a");
      tmp498 = tmp496 - tmp497;
      return NofibPrelude.atIndex(tmp498, alphas)
    } else {
      scrut3 = isSpace(c);
      if (scrut3 === true) {
        return blank
        ...
        scrut2 = isDigit(c);
        if (scrut2 === true) {
          tmp499 = NofibPrelude.int_of_char(c);
          tmp500 = NofibPrelude.int_of_char("0");
          tmp501 = tmp499 - tmp500;
          return NofibPrelude.atIndex(tmp501, digits)
        } else {
          scrut1 = c === "/";
          if (scrut1 === true) {
            return slant
            ...
            scrut = c === "=";
            if (scrut === true) {
              return NofibPrelude.reverse(slant)
            } else {
              tmp502 = lscomp(punct);
              tmp503 = NofibPrelude.Cons(NofibPrelude.Nil, NofibPrelude.Nil);
              tmp504 = NofibPrelude.Cons(NofibPrelude.Nil, tmp503);
              tmp505 = NofibPrelude.append(tmp502, tmp504);
              return NofibPrelude.head(tmp505)
            }
          }
        }
      }
...
say = function say(s) {
  let tmp493, tmp494, tmp495, tmp496;
  tmp493 = NofibPrelude.map(picChar, s);
  tmp494 = NofibPrelude.transpose(tmp493);
  tmp495 = NofibPrelude.map(join, tmp494);
  tmp496 = unlines(tmp495);
  return NofibPrelude.Cons("\n", tmp496)
};
testBanner_nofib = function testBanner_nofib(n) {
  let x, tmp493, tmp494, tmp495;
  tmp493 = NofibPrelude.nofibStringToList("Is this not a great banner?");
  x = tmp493;
  tmp494 = NofibPrelude.replicate(n, x);
  tmp495 = NofibPrelude.concat(tmp494);
  return say(tmp495)
};
...
tmp376 = NofibPrelude.Cons([
  ";",
  tmp375
], NofibPrelude.Nil);
tmp377 = NofibPrelude.Cons([
  ":",
  tmp365
], tmp376);
tmp378 = NofibPrelude.Cons([
  "+",
  tmp355
], tmp377);
tmp379 = NofibPrelude.Cons([
  "-",
  tmp345
], tmp378);
tmp380 = NofibPrelude.Cons([
  "!",
  tmp335
], tmp379);
tmp381 = NofibPrelude.Cons([
  "?",
  tmp325
], tmp380);
tmp382 = NofibPrelude.Cons([
  ".",
  tmp315
], tmp381);
punct = tmp382;
...
lambda = (undefined, function () {
  let tmp493;
  tmp493 = testBanner_nofib(1);
  return NofibPrelude.nofibListToString(tmp493)
});
